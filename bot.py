from configuration import API_ID, API_HASH, BOT_TOKEN, BOT_NAME, START_MESSAGE, HELP_MESSAGE, setLanguage, validateUrl
...
async def domainHelper(client, message):
    if not validateUrl(message.text):
        await app.send_message(message.chat.id, "Unsupported site, select /help to see what this bot does.")
        return
    await app.send_message(
